...
export class ReportsService {
  ...
  getStock(params: Record<string, unknown>): Observable<any> {
    return this.api.get<{ success: boolean; data: any }>('/reports/stock', params).pipe(map((res) => res.data));
  }

  getWarehouse(params: Record<string, unknown>): Observable<any> {
    return this.api.get<{ success: boolean; data: any }>('/reports/warehouse', params).pipe(map((res) => res.data));
  }

  getMovement(params: Record<string, unknown>): Observable<any> {
    return this.api.get<{ success: boolean; data: any }>('/reports/movement', params).pipe(map((res) => res.data));
  }

  getLowStock(params: Record<string, unknown>): Observable<any> {
    return this.api.get<{ success: boolean; data: any }>('/reports/low-stock', params).pipe(map((res) => res.data));
  }
