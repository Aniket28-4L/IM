...
export class WarehouseService {
  ...
  addStock(payload: { productId: string; warehouseId: string; qty: number; locationId?: string | null }): Observable<any> {
    return this.api.post<{ success: boolean; data: any }>('/warehouses/stock/in', payload).pipe(map((res) => res.data));
  }

  adjustStock(payload: { productId: string; warehouseId: string; qty: number; locationId?: string | null }): Observable<any> {
